...
def train():
    ...
    # Маппинг VisitorType ВРУЧНУЮ (String -> Int)
    df = df.with_columns(
    ...
    # 3. Приведение типов (ЖЕЛЕЗОБЕТОННО)
    df = df.with_columns([
    ...
    # ГЛАВНОЕ ИЗМЕНЕНИЕ: 
    # Мы сами превратили всё в числа. Категорий для CatBoost больше нет.
    cat_features = [] 
    ...
    model.fit(X, y, cat_features=cat_features) # Передаем пустой список!
